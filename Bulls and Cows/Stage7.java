...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean invalidSecretCode = false;
        int secretCodeLength = 0;

        // check the secretCodeLength is invalid or not, if it is not a number or is 0, then it is invalid
        try {
            secretCodeLength = Integer.parseInt(inputLength);
            if (secretCodeLength == 0) {
                invalidSecretCode = true;
            }
        } catch (Exception e) {
            invalidSecretCode = true;
        }

        if (!invalidSecretCode) {
            System.out.println("Input the number of possible symbols in the code:");
            int possibleRange = Integer.parseInt(sc.nextLine());

            boolean invalidPossibleRange = secretCodeLength > possibleRange || possibleRange > 36; // the possibleRange will be invalid if it is < the secretCodeLength, or it is > 37

            if (!invalidPossibleRange) {
            ...
            System.out.println(String.format("Error: \"%s\" isn't a valid number.", inputLength));
        }
