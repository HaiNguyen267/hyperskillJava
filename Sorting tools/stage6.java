...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String paraRegex = "-dataType|-sortingType|-inputFile|-outputFile";
        ...
        // check if the input para is defined
        if (cmdArgs.contains("-inputFile")) {
            int indexOfInputPara = cmdArgs.indexOf("-inputFile");
            inputFileName = cmdArgs.get(indexOfInputPara + 1);
            definedInput = true;
        }

        // check if the output para is defined
        if (cmdArgs.contains("-outputFile")) {
            int indexOfOutputPara = cmdArgs.indexOf("-outputFile");
            outputFileName = cmdArgs.get(indexOfOutputPara + 1);
            definedOutput = true;
        }
        ...
        for (String para : cmdArgs) {
            ...
            if (!para.matches(dataTypeRegex) && !para.matches(sortingTypeRegex) && !para.matches(paraRegex) && !definedInput && !definedOutput) {
        ...
        if (!undefinedDatatype && !undefinedSortingType) {
            ...
            while (sc.hasNextLine()) {
                ...
                if (dataType.equals("line")) {
                    ...
                    for (String key : parts) {
                        try {
                            long num = Long.parseLong(key); // this line just make sure the string is a number, if not this line throws exception
    ...
    private static Map<String, Integer> bubbleSort(Map<String, Integer> map) {
    ...
    public static void saveResult(String outputFileName) throws IOException {
...
//        file.createNewFile();
        ...
        writer.println(String.format("Total %s: %d.", type, total));
        ...
        if (sortingType.equals("natural")) {
            ...
            if (dataType.equals("line")) {
                writer.println(); // the lines must be printed in a new line
                for (String line : map.keySet()) {
                    for (int i = 0; i < map.get(line); i++) {
                        writer.println(line);
                    }
                ...
                for (String key : map.keySet()) {
                    for (int i = 0; i < map.get(key); i++) {
                        writer.print(key + " "); // key can be a word or a number in string format
                    }
            ...
            for (String key : map.keySet()) {
                ...
                int repeat = map.get(key);
                int percent = (int) ((double) repeat * 100) / total;;
                writer.println(String.format("%s: %d time(s), %d%%", key, repeat, percent));
            }
        }
    }
